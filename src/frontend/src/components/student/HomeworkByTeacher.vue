<template>
    <el-tabs v-model="activebook" @tab-change="refreshTab">
        <el-tab-pane :label="book.bname" :name="book.bid" v-for="book in books" :key="book.bid" lazy>
            <el-dialog title="选择页码范围" v-model="showdialog" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
                <span>从</span>
                <el-input-number v-model="start" :min="1" :controls="false" :step="1" style="width: 100px"></el-input-number>
                <span>页到</span>
                <el-input-number v-model="end" :min="1" :controls="false" :step="1" style="width: 100px"></el-input-number>
                <span>页</span>
                <template #footer>
