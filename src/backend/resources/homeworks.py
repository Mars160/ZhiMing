...
class Homeworks(restful.Resource):
    ...
    def get(self):
        ...
        treeBlob = db.session.query(SimilarityTree).order_by(SimilarityTree.sid.desc()).first()
        tree = treeBlob.getTree()

        h_qids = set()
        for wqid in wrong_qids:
            try:
                h_qid, o_qids = tree.getMostSimilar(wqid)
            except:
                print(wqid)
                tree.plot()
            if h_qid not in h_qids:
                h_qids.add(h_qid)
            else:
                h_qid = choice(o_qids)
                o_qids.remove(h_qid)
        ...
        questions = db.session.query(Question).filter(Question.qid.in_(h_qids)).all()
